...
def fill_hmmlearn_params(model: hmm.GaussianHMM, log_T: torch.tensor,
    ...
    if not torch.allclose(log_T.exp().sum(0).to(device), torch.ones(K, dtype=torch.float64).to(device)):
        log_T = log_T - ops.logsum(log_T)
    if not torch.allclose(log_t0.exp().sum().to(device), torch.ones(K, dtype=torch.float64).to(device)):
